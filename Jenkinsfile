...
void sendCommentToPullRequest(String prId, String messageContent){
     ...
     httpRequest authentication: 'sbuisson-git', httpMode: 'POST', requestBody: "${message}",  url: "https://api.github.com/repos/sbuisson/pitest-scm-test/issues/${prId}/comments"
}
...
node {
    stage('metrics') {
        ...
        def jenkinsJobUrl="http://localhost:8080/job/sbuisson/job/pitest-scm-test/view/change-requests/job/${env.BRANCH_NAME}"
            ...
            withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'sbuisson-git', usernameVariable: 'GH_LOGIN', passwordVariable: 'GH_PASSWORD']]) {
                withCredentials([[$class: 'StringBinding', credentialsId: ' git-token', variable: 'OATH']]) {
                    def githubSonarParam="-Dsonar.github.pullRequest=${prId}\
                                                        -Dsonar.github.repository=sbuisson/pitest-scm-test \
                    ...
                    messagePR+="rapport sonar : <a href='http://localhost:9000/dashboard?id=fr.perso%3Apitest-scm-test'>here</a> and <a href='${jenkinsJobUrl}//sonar_site/index.html'>here</a> <br/>"
